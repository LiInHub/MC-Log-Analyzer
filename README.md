...
- 支持批量上传日志文件（.log / .txt）
- 自动读取 Minecraft 崩溃日志、服务端日志
- AI 智能分析：精准定位崩溃原因、冲突模组、修复方案
- 纯文本展示结果
- 本地配置保存，无需重复输入 API
...
### 2. 使用步骤
1. 在「API 管理」区域输入你的 DeepSeek API Key，点击**保存 API**
2. 从下拉框选择已保存的 API
3. 点击**选择日志文件**，上传 Minecraft 日志（通常位于：`logs/latest.log` 或 `crash-reports` 文件夹）
4. 点击**开始分析日志**，等待 AI 分析完成
5. 在结果区域查看**崩溃原因、冲突模组、修复办法**

## 日志文件位置
- 客户端崩溃日志：`你的游戏目录/crash-reports/xxx.log`
- 最新运行日志：`你的游戏目录/logs/latest.log`
- 服务端日志：`服务端目录/logs/latest.log`
